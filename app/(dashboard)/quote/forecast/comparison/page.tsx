...
export default function QuoteForecastComparisonPage() {
  ...
  const gridData: ComparisonGridData[] = useMemo(() => {
    ...
    for (const [quoteNo, pair] of byQuote.entries()) {
      ...
      const estExp = Number(
        pair.est?.TotalExpense ?? Number(pair.est?.DirectExpense ?? 0) + Number(pair.est?.IndirectExpense ?? 0),
      );
      ...
      const actExp = pair.act
        ? Number(
            pair.act?.TotalExpense ?? Number(pair.act?.DirectExpense ?? 0) + Number(pair.act?.IndirectExpense ?? 0),
          )
        : null;
