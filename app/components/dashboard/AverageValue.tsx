...
const AverageValue = (props: AverageValueProps) => {
  const { title, source, date_range, date_field, stat_field, stat_value, buckets, locations } = props;
        ...
        const res = await axios.get('/api/v1/dashboard/data', {
          params: {
            ...
            filter_routes: locations
              .filter((location) => location.checked)
              .reduce(
                (a, c) => a.concat(c.routes.filter((route: any) => route.checked).map((route: any) => route.route_id)),
                []
              )
              .join(','),
          },
